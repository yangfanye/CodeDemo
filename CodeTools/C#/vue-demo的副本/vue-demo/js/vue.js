...
function toNumber (val) {
  var n = parseFloat(val); //parseFloat:将字符串转换为数字。
...
function remove (arr, item) {
  if (arr.length) {
    ...
    if (index > -1) { //-1代表没找到该值
      return arr.splice(index, 1) //数组中的该项目从数组中移除
    }
  }
}
...
function hasOwn (obj, key) {
  return hasOwnProperty.call(obj, key) //hasOwnProperty：判断对象中是否有该属性
}
