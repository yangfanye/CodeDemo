...
namespace WeiXinZhuaFaWang.lib
{
    ...
    public static class HttpHelper
    {
        ...
        {
            if (encoding == null) encoding = Encoding.UTF8;
            return InternalHttp(method, url, body, cookieContainer, encoding, others);
        }
            ...
            )
        {
            ...
            try
            {
                ...
                if (tcp.Connected)
                {
                    byte[] buff = encode.GetBytes(header);
                    if (x509certs != null)
                    {
                        using (SslStream ssl = new SslStream(tcp.GetStream(),
                                                false,
